...
2. 运行 `context_builder.py`，扫描根目录和 `knowledge/` 中的 PDF、DOCX、TXT、MD。
3. 自动生成：
...
6. 若资料满足要求，或用户选择继续运行，则生成 `temp_report.json`。
7. 等待用户确认 `Y` 后，运行 `docx_builder.py`，根据 Word 模板样式生成 `output_report.docx`。
